...
- For weekly SurveyOL invitation batching, MailerLite is legacy context rather than an active send gate. Historical MailerLite unsubscribe evidence should still be reflected in `CTS 2026` when known, but the recurring invitation-scope blocker is the current SurveyOL plus `CTS 2026` suppression state unless CTS explicitly reintroduces MailerLite into the active workflow.
...
14. After each send, reconcile SurveyOL `Opted Out`, bounced, delivery-problem, and no-send records back into `CTS 2026` and SurveyOL before the next recipient import. A SurveyOL no-send record or a registry `Do Not Email? = Yes` flag is a global CTS email suppression for future survey invitations.
    Do not let an older private MailerLite note reopen the invitation gate by accident; if MailerLite is not part of the current weekly workflow, record the suppression reconciliation against SurveyOL plus `CTS 2026`.
